...
describe("Contact Us E2E", () => {
  ...
  it("prefills Book Trial Lesson and can submit", () => {
    ...
    cy.intercept("POST", "/api/contact", { statusCode: 200, body: { success: true } }).as(
      "postContact2"
    )
  ...
  it("shows server error when POST /api/contact fails", () => {
    ...
    cy.intercept("POST", "/api/contact", { statusCode: 500, body: { error: "internal" } }).as(
      "postContactFail"
    )
